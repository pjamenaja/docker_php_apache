...
my $REPO_NAME = 'docker_php_apache';
my $POM_FILE = "$TEMP_DIR/$REPO_NAME/cloudbuild.yaml";
...
sub parse_pom
{
    ...
    while(<$fh>)
    {
        ...
        elsif (($line =~ /^\s*_PHP_VERSION\s*:\s*"(.+)"\s*$/) && (!$found_docker_version))
        { 
            #Replace only first occurence                        
            my $old_version = $1;
            $new_line =~ s/$old_version/$php_version/ig;

            print("Replaced [$old_version] with [$php_version]\n");
